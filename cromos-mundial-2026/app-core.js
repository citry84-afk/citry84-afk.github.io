...
function renderMissing(){
 const q=document.querySelector('#missingSearch').value.trim().toLowerCase(); const box=document.querySelector('#missingGroups'); box.innerHTML='';
 groupOrder().forEach(g=>{
  ...
  visible.forEach(t=>{
   ...
   row.innerHTML=`<div class="teamtop"><div><span class="teamname">${teams[t].name}</span> <span class="teamcode">${t}</span></div></div><div class="chips"></div>${(!a.length&&!u.length)?'<div class="empty">✓ Completa</div>':''}`;
   const chips=row.querySelector('.chips');
   a.forEach(n=>{const b=document.createElement('button');b.className='chip';b.textContent=codeOf(t,n);b.title='Toca para marcar como conseguido';b.onclick=()=>markGot(t,n,false);chips.appendChild(b)});

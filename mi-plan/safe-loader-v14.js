(async()=>{
  ...
  const loadScript=src=>new Promise((resolve,reject)=>{
    ...
    script.onload=resolve;script.onerror=()=>reject(new Error('No se pudo cargar '+src));document.body.appendChild(script);
  });
  removeLegacy();const cleaner=setInterval(removeLegacy,200);setTimeout(()=>clearInterval(cleaner),6000);
  try{
    const parts=Array.from({length:9},(_,i)=>`./portrait-v4-${String(i+1).padStart(2,'0')}.part?v=14`);
    ...
    (0,eval)(text.join(''));
    if(!document.querySelector('link[data-map-v5]')){const link=document.createElement('link');link.rel='stylesheet';link.href='./map-v5.css?v=14';link.dataset.mapV5='1';document.head.appendChild(link)}
    await loadScript('./map-v5.js?v=14');
    await loadScript('./visual-v10.js?v=14');
    await loadScript('./options-intuitive-v12.js?v=14');
    await loadScript('./mobile-nav-v14.js?v=14');
    try{await loadScript('./options-v14.js?v=14');}catch(error){console.warn('[ffv14] La capa avanzada no se cargó; v12 continúa disponible.',error)}
  }catch(error){

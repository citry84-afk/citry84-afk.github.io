(async()=>{
  'use strict';
  if(globalThis.__FF_BOOTSTRAP_1515__)return;
  globalThis.__FF_BOOTSTRAP_1515__=true;
  ...
  const removeOldErrors=()=>{
    [...document.querySelectorAll('body>div,body>dialog')].forEach(node=>{
      const text=(node.textContent||'').trim();
      if(text.includes('No se pudo cargar el módulo de opciones')||text.includes('No se pudo iniciar correctamente'))node.remove();
    });
  };
  ...
  try{
    ...
    const parts=Array.from({length:8},(_,i)=>`${base}/mi-plan-functional-${String(i+1).padStart(2,'0')}.part?v=15.15`);
    ...
    (0,eval)(source);
    globalThis.dispatchEvent(new CustomEvent('ff:base-ready-1515'));

    await loadScript('./v03-patch.js?v=15.15');
    globalThis.__FF_CLEAN_DASHBOARD_NAV_1513__=true;
    await loadScript('./safe-loader-v15-13.js?v=15.15');
    await loadScript('./single-nav-v15-14.js?v=15.15');
    removeOldErrors();
  }catch(error){
    console.error('[v15.15]',error);
    removeOldErrors();
    ...
    notice.textContent='No se pudo iniciar la aplicación. Esta vez el error queda registrado en la consola.';

(async()=>{
  ...
  try{
    const parts=Array.from({length:9},(_,i)=>`./portrait-v4-${String(i+1).padStart(2,'0')}.part?v=12`);
    ...
    (0,eval)(text.join(''));
    if(!document.querySelector('link[data-map-v5]')){const link=document.createElement('link');link.rel='stylesheet';link.href='./map-v5.css?v=12';link.dataset.mapV5='1';document.head.appendChild(link)}
    await loadScript('./map-v5.js?v=12');
    await loadScript('./visual-v10.js?v=12');
    await loadScript('./options-intuitive-v12.js?v=12');
  }catch(error){

(async()=>{
  ...
  try{
    const parts=Array.from({length:9},(_,i)=>`./portrait-v4-${String(i+1).padStart(2,'0')}.part?v=14.3`);
    ...
    (0,eval)(text.join(''));
    if(!document.querySelector('link[data-map-v5]')){const link=document.createElement('link');link.rel='stylesheet';link.href='./map-v5.css?v=14.3';link.dataset.mapV5='1';document.head.appendChild(link)}
    await loadScript('./map-v5.js?v=14.3');
    await loadScript('./visual-v10.js?v=14.3');
    await loadScript('./options-intuitive-v12.js?v=14.3');
    await loadScript('./mobile-nav-v14.js?v=14.3');
    try{
      await loadScript('./options-v14-3-core.js?v=14.3');
      await loadScript('./options-v14-2-core.js?v=14.3');
      window.FFv143?.migrate?.();
      await loadScript('./options-v14.js?v=14.3');
      await loadScript('./options-v14-3-ui.js?v=14.3');
      document.querySelector('.oi12-tabs [data-oi12-tab].active, .oi12-tabs [data-ffv14-tab].active')?.click();
    }catch(error){console.warn('[ffv14.3] La capa avanzada no se cargó; v12 continúa disponible.',error)}
  }catch(error){
